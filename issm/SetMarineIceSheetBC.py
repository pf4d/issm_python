...
def SetMarineIceSheetBC(md,icefrontfile=''):
  ...
  if icefrontfile:
    ...
    vertexonfloatingice[md.mesh.elements[pos].astype(int) - 1] = 1.0
  ...
  # Dirichlet Values
  if     isinstance(md.inversion.vx_obs, np.ndarray) \
     and np.size(md.inversion.vx_obs, axis=0) == md.mesh.numberofvertices \
     and isinstance(md.inversion.vy_obs, np.ndarray) \
     and np.size(md.inversion.vy_obs, axis=0) == md.mesh.numberofvertices:
  ...
  if     isinstance(md.initialization.temperature,np.ndarray) \
     and np.size(md.initialization.temperature,axis=0) == md.mesh.numberofvertices:
    ...
    if not isinstance(md.basalforcings.geothermalflux, np.ndarray) \
       or not np.size(md.basalforcings.geothermalflux,axis=0) == md.mesh.numberofvertices:
      md.basalforcings.geothermalflux = np.zeros((md.mesh.numberofvertices))
      md.basalforcings.geothermalflux[np.nonzero(md.mask.groundedice_levelset>0.)] = 50.0*10.0**(-3)    #50mW/m2
  else:
